-- Migration 041: Module Plan de Formation
-- Création du système complet de gestion des plans de formation
...
BEGIN
    ...
    SELECT 
        COUNT(*),
        COALESCE(SUM(cost_planned), 0),
        jsonb_object_agg(category, category_stats)
    INTO total_count, total_cost, by_category
    FROM (
        SELECT 
            category,
            jsonb_build_object('count', COUNT(*), 'cost', COALESCE(SUM(cost_planned), 0)) as category_stats
    ...
    FROM (
        SELECT 
            trimestre_cible,
            jsonb_build_object('count', COUNT(*), 'cost', COALESCE(SUM(cost_planned), 0)) as quarter_stats
        FROM plan_formations 
        WHERE annee = target_annee AND phase = 'PREVISIONNEL' AND statut = 'VALIDE'
    ...
    FROM (
        SELECT 
            mois_cible,
            jsonb_build_object('count', COUNT(*), 'cost', COALESCE(SUM(cost_planned), 0)) as month_stats
        FROM plan_formations 
        WHERE annee = target_annee AND phase = 'PREVISIONNEL' AND statut = 'VALIDE'
        GROUP BY mois_cible
    ) month_stats;
