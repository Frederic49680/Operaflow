-- Migration 048: Données de base pour la gestion des utilisateurs (version 2)
-- Basé sur le PRD1.mdc

-- Vérifier que les tables existent avant d'insérer les données
DO $$
BEGIN
    -- Insérer les rôles par défaut seulement si la table existe
...
-- Assigner les permissions aux rôles seulement si les tables existent
DO $$
BEGIN
...
-- Règles d'accès par page seulement si les tables existent
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'page_access_rules') 
